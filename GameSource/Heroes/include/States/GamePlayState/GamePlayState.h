	...
	namespace States
	{
		namespace GamePlay
		{
			...
			private:
				...
				void QuerySimulationZone(std::list<EntityIDType>& entityList);
				void QueryVisionZone(std::list<EntityIDType>& entityList);
				void RenderEntites(std::list<EntityIDType> entityList);
				float GetRotationAngle(b2Vec2& rotationVector);

				// temporary point for camera to follow
				b2Vec2 m_simulationPoint{ b2Vec2_zero };

				std::list<EntityIDType> m_entityList;

				GamePlay::EntityMemory m_entityMemory;
