...
void EntityTargetMap::AddTargetFront(int target, EntityMemory& entityMemory)
{
    if (CheckTarget(target, entityMemory))
    {
        ...
        if (newEntry->m_dynamicID == -1)
        {
            ...
            {
                newEntry->m_before = NULL;
                newEntry->m_after = NULL;
            ...
            else
            {
                ...
                while (insertionPoint->m_value < newEntry->m_value)
                {
                    ...
                    // if this is the end
                    if (insertionPoint->m_after == NULL)
                    {
                        end = true;
                        break;
                    }
...
void EntityTargetMap::RemoveEntryFromInternalList(TargetMapEntry* entry)
{
    ...
    {
        // target is first and last
        if (entry->m_before == NULL && entry->m_after == NULL)
        {
            m_targetFront = NULL;
            m_targetBack = NULL;
        }
        // just first
        else if (entry->m_before == NULL)
        {
            m_targetFront = entry->m_after;
            entry->m_after->m_before = NULL;
        }
        // just last
        else if (entry->m_after == NULL)
        {
            // no back to set
            m_targetBack = entry->m_before;
            entry->m_before->m_after = NULL;
        }
        // middle
        else
        {
